...
const CounterAdvanced = () => {
  ...
  const increaseCount = () => {
    ...
    setState({
      count: state.count + 1,
      lastAction: "Increase", time: getCurrentTime()
    })
  ...
  const decreaseCount = () => {
    if( state.count > 0 ) {
      setState({
        count: state.count - 1,
        lastAction: "Decrease", time: getCurrentTime()
      })
    }
  }
  ...
  const resetCount = () => {
    if( state.count > 0 ) {
      setState({
        count: 0,
        lastAction: "Reset", time: getCurrentTime()
      })
    }
  }
    ...
    <>
      <div className="space-y-4 text-2xl pt-12">
        ...
        <div className="text-center space-x-4">
          <CounterButton onClick={increaseCount}  label="Increace"/>
